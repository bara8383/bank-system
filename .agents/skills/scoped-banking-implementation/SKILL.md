...
`implementer` に対応する repo-local skill として、バンクシステム開発で繰り返す「planner の accepted scope のみを安全に実装」を一定品質で実行する。cycle は `planner` -> `implementer` -> reviewer 群の直列で進め、implementer は planner の後に同一 cycle の accepted scope を実装する。直近の完了済み cycle の accepted scope を使うのは、再実行や復旧などの例外時だけとする。
...
4. 同一 cycle に accepted scope が存在しない、または空の場合は原則として実装しない。再実行や復旧などの例外時だけ、直近の完了済み cycle の accepted scope を fallback として読める。
...
3. 同一 cycle に accepted scope がない場合は `docs/ai/cycles/<cycle-id>/implementer.md` に `blocked: accepted scope not found` と実装しなかった理由を書く。再実行や復旧などの例外時だけ、直近の完了済み cycle の accepted scope を fallback として使える。
4. 学習用ミニバンキングシステムの範囲内か確認する。本番金融システム相当の断定は避ける。
5. 金額、残高、取引履歴、監査ログ、冪等性、認証認可、DBトランザクション境界への影響を確認する。
6. 未確定の仕様は「作業仮定」として分離し、PR レビューで確認しやすい小さな差分にする。
...
出力は原則として次の区分を使う: 参照した accepted scope、変更内容、scope 適合性、実装しなかったこと、テスト結果、作業仮定。
